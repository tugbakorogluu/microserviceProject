...
async function bootstrap() {
  ...
  try {
    ...
    console.log('✅ Package Service RabbitMQ microservice connected');
  } catch (err) {
    // RabbitMQ hazır değilse HTTP sunucusu yine de ayakta kalır
    console.warn(
      `⚠️ Package Service RabbitMQ connection failed, HTTP server will still run: ${err.message}`,
    );
  }
  ...
  console.log(`✅ Package Service HTTP server running on port ${httpPort}`);
}
